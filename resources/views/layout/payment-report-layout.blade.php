      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="row">
			...
			@foreach ($paymentTransaction as $rd)
                        <tr>
                          <td>{{$rd->transaction_id}}</td>
                          <td>{{$rd->email}}</td>
                          <td>{{ number_format($rd->amount,2)}}</td>
                          <td>{{ number_format($rd->amount_due,2)}}</td>
                          <td>{{$rd->transaction_status}}</td>
                          <td>{{$rd->created_at}}</td>
                        </tr> 
                        @endforeach
		@else
		<tr>
			<td colspan="8">Payment transaction not available.</td>
		</tr>
		@endif                       
                      </tbody>
                    </table>
                    {{ $paymentTransaction->links()}}
                  </div>
                </div>
