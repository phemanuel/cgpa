              ...
              <div class="card">
                <div class="card-header">
                  ...
                  <a href="{{route('semester-result')}}"  class="btn btn-outline-primary">
                    ...
                    <button class="btn btn-success" onclick="exportTableToCSV('semester_results.csv')">
...
<table class="table table-bordered">
    <thead>
        {{-- Report Header Row --}}
        <tr>
            <th colspan="{{ 4 + count($ctitleKeys) }}" class="text-center">
        ...
        @forelse ($results as $index => $result)
            <tr>
                ...
                @foreach ($ctitleKeys as $key)
                    ...
                    @endphp
                    <td>{{ is_numeric($score) ? (fmod($score, 1) === 0.0 ? intval($score) : $score) : $score }}</td>
                @endforeach

                <td>{{ is_numeric($result->gpa) ? (fmod($result->gpa, 1) === 0.0 ? intval($result->gpa) : $result->gpa) : $result->gpa }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="{{ 4 + count($ctitleKeys) }}">No results found.</td>
            </tr>
        @endforelse
    </tbody>
</table>
