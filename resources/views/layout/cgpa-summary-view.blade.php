...
<body>
  ...
  <div id="app">
   ...
   <div class="container">
    ...
    @if(!empty($cgpaData) && count($cgpaData))
    <div class="table-responsive" id="printArea">
        <table class="table table-bordered">
            <thead>
                {{-- Report Header --}}
                <tr>
                    <th class="text-center" colspan="{{ 6 + (count($levels) * 3) }}">
                        {{ $programme }} - {{ $acadsession }} - CGPA Report
                    </th>
                </tr>
                ...
                <tr>
                    ...
                    {{-- Loop levels with semesters + yearly CGPA --}}
                    @foreach($levels as $level)
                        <th>{{ $level }} (GPA 1st)</th>
                        <th>{{ $level }} (GPA 2nd)</th>
                        <th>{{ $level }} (CGPA Year)</th>
                    @endforeach
            ...
            <tbody>
                @foreach($cgpaData as $index => $data)
                    @php
                        $student   = $data['student'];
                        $yearly    = $data['yearly'];
                        $finalCgpa = $data['final'];
                        $totalGP   = $data['total_grade_points'] ?? 0;
                        $totalCU   = $data['total_course_units'] ?? 0;
                    @endphp
                    ...
                    <tr>
                        ...
                        {{-- Loop through each level/semester GPA --}}
                        @foreach($levels as $level)
                            @php
                                $first  = collect($data['semesters'])->firstWhere(fn($s) => $s['level'] === $level && $s['semester'] === 'First')['gpa'] ?? '-';
                                $second = collect($data['semesters'])->firstWhere(fn($s) => $s['level'] === $level && $s['semester'] === 'Second')['gpa'] ?? '-';
                                $year   = $yearly[$level] ?? '-';
                            @endphp

                            <td>{{ is_numeric($first) ? number_format($first, 2) : '-' }}</td>
                            <td>{{ is_numeric($second) ? number_format($second, 2) : '-' }}</td>
                            <td>{{ is_numeric($year) ? number_format($year, 2) : '-' }}</td>
                        @endforeach
                        ...
                        @php
                            ...
                            if (is_numeric($cgpa)) {
                                if ($cgpa >= 3.5) {
                                    $cgpaClass = 'badge bg-success text-white';
                                    $cgpaRemark = 'Distinction';
                                } elseif ($cgpa >= 3.0) {
                                    $cgpaClass = 'badge bg-primary text-white';
                                    $cgpaRemark = 'Upper Credit';
                                } elseif ($cgpa >= 2.5) {
                                    $cgpaClass = 'badge bg-info text-white';
                                    $cgpaRemark = 'Lower Credit';
                                } elseif ($cgpa >= 2.0) {
                                    $cgpaClass = 'badge bg-warning text-white';
                                    $cgpaRemark = 'Pass';
                                } else {
                                    $cgpaClass = 'badge bg-danger text-white';
                                    $cgpaRemark = 'Fail';
                                }
                            }

                            $formattedCgpa = is_numeric($cgpa) ? number_format($cgpa, 2) : '-';
                        @endphp

                        <td><span class="{{ $cgpaClass }}">{{ $formattedCgpa }}</span></td>
                        <td>{{ $cgpaRemark }}</td>
                    </tr>
                @endforeach
            </tbody>
        ...
        {{-- GPA Grading Scale --}}
        <div class="mt-3">
            <strong>CGPA Grading Scale:</strong>
    ...
    @else
        <div class="alert alert-warning">
            No CGPA summary data available for {{ $programme }} ({{ $acadsession }}).
        </div>
    @endif
